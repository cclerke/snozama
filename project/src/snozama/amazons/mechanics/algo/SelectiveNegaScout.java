...
public class SelectiveNegaScout {
	...
	public int NegaScoutSearch(Board board, int depth, int maxDepth, int alpha, int beta, int colour, int turn)
	{
		...
		if (next == 0 && depth == 0 && maxDepth > 1)
		{
			...
			trimSearchSpace(successors, scores);
		}
	...
	private void trimSearchSpace(MoveManager moves, int[] scores)
	{
		int cursor = 0;
		...
		// If the cursor got all the way to the end.
		if (cursor > 0 && cursor < (moves.size()-1) &&  (((double)cursor) / ((double)moves.size())) > 0.25)
		{
			System.out.println("We are trimming here...");
			moves.setSize(cursor);
		}
	}
}
